...
// astructure to represent a connected, undirected
// and unweighted as a collection of edges.
struct Graph
{
    ...
    // to src. Both counted as 1 edge here.
    Edge* edge;
};
...
// Function prototypes for union-find (These functions are defined after kargerMinCut() )
int find(struct subset subsets[], int i);
void Union(struct subset subsets[], int x, int y);

// A very basic implementation of Karger's randomised
// algorithm for finding the minimum cut. Please not that 
// Karger's algorithm is a Monte Carlo Randomised algorithm
// and the cut returned by the algorithm may not be minimum always
...
int kargerMinCut(struct Graph* graph)
{
    ...
    // Now we have two vertices (or subsets) left in the contracted graph, so count the edges between
...
// A utility function to finde set of an element i

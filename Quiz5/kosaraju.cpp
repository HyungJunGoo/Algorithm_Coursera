// C++ implementation of Kosafaju's algorithm to print all SCCs
#include <iostream>
#include <list>
#include <stack>
...
class Graph
{
    ...
    void DFSUtill(int v, bool visitedp[]);
    public:
        Graph(int V);
...
Graph Graph::getTranspose()
{
    ...
    for (int v = 0; v < V; v++)
    {
        // Recur for all the vertices adjacent to this v
        list<int>::iterator i;
        for (i = adj[v].begin(); i != adj[v].end(); ++i)
            g.adj[*i].push_back(v);
    }
...
void Graph::printSCCs()
{
    ...
    // Mark all the verticdes as not visited (For first DFS)
